...
procedure TessError(errn: GLenum); cdecl; // Эта функция вызывается в процессе триангуляции с помощью Glu только тогда, когда возникает ошибка триангуляции
var
  str:PChar;
begin
...
procedure TessCombineUserData(vrtx:PValue3d; vrtx_data:Pointer; weight:Pointer; out dataout:PDouble; usrData:Pointer); cdecl;
var
  i:Integer;
...
var
  ...
  CrossVertex:TArrayOfValue3d; // Массив для точек, которые получаются при пересечении
...
begin
  ...
  if ffill then // если необходимо заполнять шрифты, то выполняем необходимые действия для запуска триангуляции с помощью библиотеки GLU
    begin
      ...
      gluTessCallback(gluTess, GLU_TESS_COMBINE_DATA, @TessCombineUserData);
      gluTessCallback(gluTess, GLU_TESS_VERTEX, glVertex3dv);
      gluTessCallback(gluTess, GLU_TESS_BEGIN, glBegin);
      gluTessCallback(gluTess, GLU_TESS_END, glEnd);
      gluTessCallback(gluTess, GLU_TESS_ERROR, @TessError);

      gluTessProperty(gluTess, GLU_TESS_WINDING_RULE, GLU_TESS_WINDING_NONZERO);
      gluTessProperty(gluTess, GLU_TESS_BOUNDARY_ONLY, 0);
    end;
              ...
              for ic:=0 to glfq.ccnt-1 do // Цикл по контурам
                begin
                  ...
                  for icp:=0 to  glfq.circs[ic].pcnt-1 do
                    gluTessVertex(gluTess, TGLArrayd3(glfq.circs[ic].pts[icp]), @glfq.circs[ic].pts[icp]);
