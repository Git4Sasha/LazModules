...
begin
  ...
  repeat
    ...
    if (res=0)and(bread<>0) then begin                                         // Если данные приняты, то
      ...
      if FUseRecvRingBuf then begin                        // Если изпользуется кольцевой буфер, то данные помещаются в кольцевой буфер
...
begin
  ...
  libusb_attach_kernel_driver(FDevHandle, 0); // отдаём системе, захваченное ранее USB устройство
  libusb_close(FDevHandle);
