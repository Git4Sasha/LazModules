...
uses
  Classes, SysUtils, ctypes, LCLProc, LCLType, X, XUtil, XLib,
  InterfaceBase,
  glx,
  WSLCLClasses,
  {$IFDEF LCLGTK2}
...
begin
  ...
  if (sharelist<>nil) then
    glxcontext := glXCreateContext(dpy, vi, PrivateShareList^.glxcontext, direct)
  else
...
begin
  ...
  if not GTK_IS_GL_AREA(glarea) then
    RaiseGDBException('LOpenGLMakeCurrent not a PGtkGLArea');
